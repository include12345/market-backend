...
public class PermissionUtil {
    ...
    public static List<PermResult> listPermResult(List<Permission> permissions) {
        List<PermResult> root = new ArrayList<>();
        for (Permission permission : permissions) {
            ...
            PermResult perm1 = null;
            for (PermResult PermResult : root) {
                if (PermResult.getLabel().equals(menu1)) {
                    perm1 = PermResult;
                    break;
                }
            }
            if (perm1 == null) {
                perm1 = new PermResult();
                perm1.setId(menu1);
            ...
            PermResult perm2 = null;
            for (PermResult PermResult : perm1.getChildren()) {
                if (PermResult.getLabel().equals(menu2)) {
                    perm2 = PermResult;
                    break;
                }
            }
            if (perm2 == null) {
                perm2 = new PermResult();
                perm2.setId(menu2);
            ...
            PermResult leftPerm = null;
            for (PermResult PermResult : perm2.getChildren()) {
                if (PermResult.getLabel().equals(button)) {
                    leftPerm = PermResult;
                    break;
                }
            }
            if (leftPerm == null) {
                leftPerm = new PermResult();
